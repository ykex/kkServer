...
const int DEFAULT_KEEP_ALIVE_TIME = 10 * 60; //s
...
void defaultHttpCallback(const HttpRequest &req, HttpResponse *resp){
    ...
    if(!benchmark){const std::map<std::string, std::string>& headers = req.headers();
    ...
    if (req.path() == "/"){
        ...
        resp->addHeader("Server", "KK");
        time_t now = time(0);
        std::string dt(ctime(&now));
        resp->setBody("<html><head><title>This is title</title></head>"
            "<body><h1>Hello</h1>Now is " + dt +
            "</body></html>");
    }
...
void HttpServer::onRequest(const ConnectionPtr &conn, const HttpRequest &req){
    ...
    if(connection=="Keep-Alive")
        conn->setTimer(DEFAULT_KEEP_ALIVE_TIME);
